...
<div class="card">
    ...
    <div class="card-body">
        <div class="row">
            <input type="hidden" name="pedidosid" value="<?php echo e($pedido->pedidosid); ?>" id="pedidosid">
            ...
            <div class="form-group col-md-4 col-sm-6">
                ...
                <select class="form-control form-control-sm aiz-selectpicker" name="clientes_sucursalesid">
                    ...
                    <option value="<?php echo e($sucursal->clientes_sucursalesid); ?>" <?php if($sucursal->clientes_sucursalesid ==
                        $pedido->clientes_sucursalesid): ?> selected <?php endif; ?>>
            ...
            <div class="form-group col-md-4 col-sm-6">
                ...
                <input type="text" class="form-control form-control-sm" name="destinatario" id="destinatario"
                    placeholder="Ingrese Destinatario" autocomplete="off" value="<?php echo e($destinatario); ?>">
            </div>
            ...
            <div class="form-group col-md-4 col-sm-6">
                ...
                <select class="form-control form-control-sm aiz-selectpicker" name="centros_costosid">
                    ...
                    <option <?php if($centro_costo->centros_costosid == $pedido->centros_costosid): ?> selected <?php endif; ?>
                        value=" <?php echo e($centro_costo->centros_costosid); ?>">
            ...
            <div class="form-group col-md-4 col-sm-6">
                <label for="sucursal" class="col-form-label">Estado</label>
                <select class="form-control form-control-sm aiz-selectpicker" name="estado" id="estado">
                    <option value="1" <?php if(isset($pedido->estado)): ?><?php if($pedido->estado==1 ): ?> selected <?php endif; ?>
                        <?php endif; ?>>Realizado
                    </option>
                    <option value="2" <?php if(isset($pedido->estado)): ?><?php if($pedido->estado==2 ): ?> selected <?php endif; ?>
                        <?php endif; ?>>Confirmado
                    </option>
                    <option value="3" <?php if(isset($pedido->estado)): ?><?php if($pedido->estado==3 ): ?> selected <?php endif; ?>
                        <?php endif; ?>>Facturado
                    </option>
                    <option value="4" <?php if(isset($pedido->estado)): ?><?php if($pedido->estado==4 ): ?> selected <?php endif; ?> <?php endif; ?>>En
                        Entrega
                    </option>
                    <option value="5" <?php if(isset($pedido->estado)): ?><?php if($pedido->estado==5 ): ?> selected <?php endif; ?>
                        <?php endif; ?>>Entregado
                    </option>
                    <option value="6" <?php if(isset($pedido->estado)): ?><?php if($pedido->estado==6 ): ?> selected <?php endif; ?> <?php endif; ?>>No
                        Aplica
                    </option>
                </select>
            </div>
            ...
            <div class="form-group col-md-4 col-sm-6">
                <label for="sucursal" class="col-form-label">Prioridad</label>
                <select class="form-control form-control-sm aiz-selectpicker" name="prioridad" id="prioridad">
                    <option value="0" <?php if(isset($pedido->prioridad)): ?><?php if($pedido->prioridad==0 ): ?> selected <?php endif; ?>
                        <?php endif; ?>>No
                    </option>
                    <option value="1" <?php if(isset($pedido->prioridad)): ?><?php if($pedido->prioridad==1 ): ?> selected <?php endif; ?>
                        <?php endif; ?>>Si
                </select>
            </div>

            <div class="form-group col-md-4 col-sm-6">
                <label for="urbano" class="col-form-label">Urbano</label>
                <input type="text" class="form-control form-control-sm" name="urbano" id="urbano"
                    placeholder="Ingrese Codigo de Urbano" autocomplete="off" value="<?php echo e($urbano); ?>">
            </div>
...
<!-- Añade esto junto a los otros modales -->
...
<script>
    var detallesPedido = <?php echo json_encode($detalles, 15, 512) ?>;
    $(document).ready(function() {
        ...
        var pedidosTable = new Tabulator("#pedidos-table", {
            ...
            data: detallesPedido, // Inicialmente la tabla estará vacía
            columns: [
                {
                    title: "Información",
                    field: "informacion",
                ...
                { title: "Código", field: "productocodigo", width: 110 },
                {
                    title: "Descripción",
                    field: "descripcion",
                    widthGrow: 3,  // Esta columna crecerá más, ocupando más espacio proporcional
                    minWidth: 200  // Establece un ancho mínimo para asegurar buena visibilidad del texto
                },
                {
                    title: "Medida",
                    field: "medida",
                ...
                },
                {
                    ...
                    cellEdited: function(cell) {
                        let row = cell.getRow();
                        let oldValue = parseFloat(cell.getOldValue()).toFixed(2); // Redondear el valor anterior
                        let newValue = parseFloat(cell.getValue()).toFixed(2); // Redondear el valor nuevo

                        // Solo actualiza 'cantidadanterior' si el valor realmente cambió
                        if (oldValue !== newValue) {
                            row.update({ cantidadanterior: oldValue });

                            // Recalcula el total de la fila
        ...
        // Recalcular los totales para cada fila y los totales generales al iniciar la página
        pedidosTable.on("tableBuilt", function() {
            // Recorre todas las filas para calcular el total de cada una
            pedidosTable.getRows().forEach(function(row) {
                calcular_total_fila(row);
            });

            // Calcula los totales generales después de haber calculado cada fila
            calcular_total();
        });
        ...
        // Manejar el guardado de la observación
       ...
       // Función para calcular el total de una fila
        ...
        $('#btnGuardar').on('click', function (event) {
            ...
            $.ajax({
                url: '<?php echo e(route('pedidos.actualizar')); ?>',  // La URL a la que enviarás los datos.
                type: 'POST',
                data: {
...
<?php echo $__env->make('backend.layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\laragon\www\tienda\resources\views/backend/pedidos/editar.blade.php ENDPATH**/ ?>